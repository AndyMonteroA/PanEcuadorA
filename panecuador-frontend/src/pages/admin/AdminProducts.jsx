...
export default function AdminProducts() {
  ...
  const openEdit = (product) => {
    setForm({
      ...
      id_productor: product.id_productor || '',
      disponible: product.disponible,
      tiempo_elaboracion_min: product.tiempo_elaboracion_min || '30',
      vida_util_dias: product.vida_util_dias || '3',
      complejidad: product.complejidad || '3',
      num_ingredientes: product.num_ingredientes || '5'
    });
    setImageFile(null);
  ...
  return (
    <div>
      ...
      {alert && (
        <div className={`admin-alert admin-alert-${alert.type}`}>{alert.message}</div>
      )}
          ...
          <>
            <table className="admin-table">
              ...
              <tbody>
                ...
                {products.length === 0 && (
                  <tr><td colSpan={6} className="admin-empty">No se encontraron productos</td></tr>
                )}
              </tbody>
            </table>
            ...
            {pagination.totalPages > 1 && (
              <div className="admin-pagination">
                <button disabled={pagination.page <= 1} onClick={() => loadProducts(pagination.page - 1, search)}>
                  Anterior
                </button>
                <span className="admin-pagination-info">
                  Página {pagination.page} de {pagination.totalPages}
                </span>
                <button disabled={pagination.page >= pagination.totalPages} onClick={() => loadProducts(pagination.page + 1, search)}>
                  Siguiente
                </button>
              </div>
            )}
          ...
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="admin-modal-actions">
                <button type="button" className="btn-admin btn-admin-ghost" onClick={() => setModal({open:false,mode:'create',data:null})}>
                  Cancelar
                </button>
