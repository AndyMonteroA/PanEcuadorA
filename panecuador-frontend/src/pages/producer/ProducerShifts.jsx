...
export default function ProducerShifts() {
  ...
  async function fetchAll() {
    ...
    try {
      const [w, s, a] = await Promise.all([
        ...
        producerAPI.getShiftsList(),
        producerAPI.getShiftAssignments({ fecha_inicio: weekDates[0], fecha_fin: weekDates[6] }),
      ]);
      setWorkers(w.data.data.filter(worker => worker.activo));
  ...
  const handleAssign = async (e) => {
    ...
    setSaving(true);
    try {
      await producerAPI.createShiftAssignment({
        id_trabajador: parseInt(form.id_trabajador),
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await producerAPI.deleteShiftAssignment(id);
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-page-header" style={{ marginBottom: '24px' }}>
        <div>
          <h1 className="admin-page-title">🕐 Gestión de Turnos del Personal</h1>
          <p style={{ color: 'var(--text-muted)', marginTop: '4px', fontSize: '0.9rem' }}>
            Organiza los turnos de tus panaderos y pasteleros para los próximos 7 días
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: '24px', alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: '24px', overflowX: 'auto' }}>
          ...
          {workers.length === 0 ? (
            <div className="empty-state"><span className="empty-emoji">👷</span><h3>No hay trabajadores registrados en tu panadería</h3></div>
          ) : (
